...
const ChatRoom = ({ groupId, currentUser }) => {
    ...
    const fetchGroup = async () => {
      try {
        const res = await axios.get(
          `https://3b1d208aefe0.ngrok-free.app/api/groups/${groupId}/user/${currentUser._id}`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await axios.get(
          `https://3b1d208aefe0.ngrok-free.app/api/users`,
          {
            headers: {
              'ngrok-skip-browser-warning': 'true',
            },
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const res = await axios.get(
          `https://3b1d208aefe0.ngrok-free.app/api/messages/${groupId}`,
          {
            headers: {
              'ngrok-skip-browser-warning': 'true',
            },
  ...
  const handleSend = async () => {
    if (text.trim()) {
      ...
      try {
        await axios.post(
          'https://3b1d208aefe0.ngrok-free.app/api/messages',
          message,
          {
            headers: {
              'ngrok-skip-browser-warning': 'true',
  ...
  const handleAddUser = async (userId) => {
    try {
      await axios.post(
        `https://3b1d208aefe0.ngrok-free.app/api/groups/${groupId}/add-user`,
        {
          userId,
